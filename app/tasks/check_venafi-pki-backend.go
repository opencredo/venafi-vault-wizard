...
func CheckVenafiPKIBackend(input *CheckVenafiPKIBackendInput) error {
	...
	err := checks.VerifyVenafiSecret(configurePluginSection, input.VaultClient, fmt.Sprintf("%s/venafi/%s", input.PluginMountPath, input.SecretName), input.SecretValue["zone"].(string))
	if err != nil {
		return err
	}
